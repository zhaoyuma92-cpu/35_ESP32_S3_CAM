...
### 新增 — 曝光自动收敛后锁定为固定值，HTTP接口预留手动调整能力

- 背景：之前实测确认AEC/AGC在我们的测试条件下全程未变化，但那只是因为光照
  稳定、没有触发调整；如果实际部署现场光照会变化，AEC仍可能被真正触发，
  引入额外的不确定性。这次改为主动锁定，排除这个变量。
- `p4_camera.c/.h`：新增 `p4_camera_get_exposure()`/`p4_camera_set_exposure()`，
  基于 `0x3503`（AEC/AGC手动模式开关）+ `0x3500-02`（曝光行数）+
  `0x350a-0b`（增益）寄存器读写
- `p4_camera_init()` 末尾：CSI开始接收帧后等待2秒让AEC在当前光照下收敛，
  读出收敛值后立即调用 `p4_camera_set_exposure()` 锁定，不再被自动调整；
  不持久化到NVS，每次开机都重新走一遍收敛流程
- `http_api.c`：`GET /api/config` 返回里增加 `exposure_lines`/`exposure_gain`
  当前值；`POST /api/config` 接受这两个字段并立即调用 `p4_camera_set_exposure()`
  生效（曝光是传感器实时状态，不进 `app_config_t` 快照），为以后主控
  dashboard加手动曝光调整UI预留好节点侧接口
